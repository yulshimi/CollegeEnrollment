        ...
        <tr>
            ...
            <%
                    ...
                    if(action != null && action.equals("Submit")) 
                    {
                        ...
                        stmt2 = conn.prepareStatement("SELECt count(ct.name) as completed from concentration ct, take t, student s, class c, grade_conversion gc where s.id = t.id      and s.id = ? and c.course_no = ct.course_no and ct.degree_name = s.degree and t.class_id = c.class_id and t.grade != 'IC' and gc.letter_grade = t.grade and (gc.number_grade >= min_gpa or t.grade_option = 'S/U') group by ct.name,ct.min_units having sum(t.unit) >= ct.min_units");
                        stmt2.setInt(1, Integer.parseInt(request.getParameter("ID")));
                        ResultSet rs2 = stmt2.executeQuery();
                        
                        stmt3 = conn.prepareStatement("Select c.course_no, cls.quarter from concentration c, class cls where c.course_no not in (SELECt ct.course_no from concentration ct, take t, student s, class c, grade_conversion gc       where s.id = t.id      and s.id = ? and c.course_no = ct.course_no and ct.degree_name = s.degree and t.class_id = c.class_id and t.grade != 'IC' and gc.letter_grade = t.grade and (gc.number_grade >=      min_gpa or t.grade_option = 'S/U')) and cls.instructor is NULL and cls.course_no = c.course_no"
                        );
            ...
            %>
                <table border="1">
                <form action="Report1d2.jsp" method="get">
                    <select name="ID">
